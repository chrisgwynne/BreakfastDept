...
// Mirrors the /breakfast-admin/api/v1/portfolio shapes. The list rows are the
// server "summariseRow" projection; the full record carries its child entities.
...
export type PortfolioCounts = Record<string, number>
...
// The full record is a flat map of the portfolio_records columns plus its
// child collections. Kept loose (index signature) so new editorial columns
// don't require a type change to round-trip through the editor.
...
// validateForPublish returns a flat list of human-readable blocker messages.
export type PortfolioBlockers = string[]
