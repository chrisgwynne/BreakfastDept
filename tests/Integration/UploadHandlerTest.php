...
final class UploadHandlerTest extends PlatformTestCase
{
    ...
    {
        $handler = new UploadHandler($this->platform->db(), $this->platform->uploadsDir(), ['enabled' => false]);
    ...
    {
        $handler = new UploadHandler($this->platform->db(), $this->platform->uploadsDir(), ['enabled' => true, 'maxBytes' => 1048576]);
        ...
        $row = $this->platform->db()->one('SELECT * FROM uploads WHERE uuid = :u', ['u' => $res['upload']['uuid']]);
    ...
    {
        $handler = new UploadHandler($this->platform->db(), $this->platform->uploadsDir(), ['enabled' => true]);
    ...
    {
        $handler = new UploadHandler($this->platform->db(), $this->platform->uploadsDir(), ['enabled' => true, 'maxBytes' => 8]);
    ...
    public function testInfectedFileRejectedByScanner(): void
    {
        ...
        $handler = new UploadHandler($this->platform->db(), $this->platform->uploadsDir(), [
            'enabled' => true,
